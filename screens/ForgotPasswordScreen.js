...
export default function ForgotPasswordScreen({ navigation }) {
  ...
  const handlePasswordReset = async () => {
    ...
    if (!e) return setErrorMsg("Veuillez entrer votre adresse e-mail.");
    if (!isEmail(e)) return setErrorMsg("Format d’e-mail invalide.");
    if (cooldown > 0) return;

    try {
      setSending(true);
      await sendPasswordResetEmail(auth, e);
      setSuccessMsg("Un lien de réinitialisation a été envoyé. Vérifiez votre boîte de réception et les spams.");
      startCooldown(30);
    } catch (error) {
      switch (error.code) {
        case "auth/user-not-found":
          setErrorMsg("Aucun compte n’est associé à cet e-mail. Vérifiez l’adresse ou créez un compte.");
          break;
        ...
        case "auth/missing-android-pkg-name":
          setErrorMsg("Lien de redirection mal configuré (Domaines autorisés / continueUrl).");
          break;
        default:
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        <TouchableOpacity
          style={[styles.resetBtn, (sending || cooldown > 0 || !isEmailValid) && { opacity: 0.6 }]}
        ...
        <View style={{ marginTop: 12 }}>
          ...
          <Text style={styles.hint}>• L’e-mail doit correspondre à un compte existant (Authentication → Utilisateurs)</Text>
        </View>
      </View>
    </ImageBackground>
  );
...
const styles = StyleSheet.create({
  ...
  resetBtn: { backgroundColor: "#6C63FF", padding: 15, borderRadius: 25, alignItems: "center", marginBottom: 14 },
