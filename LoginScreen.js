...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // onAuthStateChanged (App.js) fera la navigation vers Home automatiquement
    } catch (error) {
      // message lisible dans l’UI
      setErrors((prev) => ({ ...prev, global: "Identifiants invalides ou compte introuvable." }));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("SignUp")}>
          <Text style={styles.signupText}>
            ...
            <Text style={{ color: "#6C63FF", fontWeight: "bold" }}>Sign Up</Text>
          </Text>
        </TouchableOpacity>
      </View>
    </ImageBackground>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: "#f5f5f5", padding: 12, borderRadius: 10, marginBottom: 8, fontSize: 16, borderWidth: 1, borderColor: "#ddd" },
  inputError: { borderColor: "#e74c3c", backgroundColor: "#fff" },
  errorText: { color: "#e74c3c", marginBottom: 8, fontSize: 13 },
  errorGlobal: { color: "#e74c3c", textAlign: "center", marginBottom: 12 },
  forgot: { color: "#6C63FF", textAlign: "right", marginVertical: 8 },
  loginBtn: { backgroundColor: "#6C63FF", padding: 15, borderRadius: 25, alignItems: "center", marginTop: 8, marginBottom: 16 },
